...
#define IMX334_PIXEL_RATE_WITH_445M_10BIT	(IMX334_LINK_FREQ_445 * 2 / 10 * 4)
#define IMX334_PIXEL_RATE_WITH_594M_12BIT	(IMX334_LINK_FREQ_594 * 2 / 12 * 4)
#define IMX334_PIXEL_RATE_WITH_891M_10BIT	(IMX334_LINK_FREQ_891 * 2 / 10 * 4)
#define IMX334_PIXEL_RATE_WITH_891M_12BIT	(IMX334_LINK_FREQ_891 * 2 / 12 * 4)

#define IMX334_XVCLK_FREQ_37		18000000 //37125000
#define IMX334_XVCLK_FREQ_74		74250000

#define IMX334_CHIP_ID				0x3000
#define IMX334_REG_IMX334_CHIP_ID		0x302c

#define IMX334_REG_CTRL_MODE		0x3000
#define IMX334_MODE_SW_STANDBY		0x1
#define IMX334_MODE_STREAMING		0x0

#define imx334_REG_MARSTER_MODE		0x3002
#define imx334_MODE_STOP		BIT(0)
#define imx334_MODE_START		0x0
...
struct imx334_mode {
	...
	u32 vts_def;
	u32 exp_def;
	const struct imx334_regval *global_reg_list;
	const struct imx334_regval *reg_list;
	u32 hdr_mode;
	u32 vclk_freq;
	u32 bpp;
	u32 mipi_freq_idx;
...
static const struct imx334_regval imx334_10_3840x2160_global_regs[] = {
	{0x3001, 0x00},
	{0x3002, 0x00},
	{0x300C, 0x2D},// BCWAIT_TIME[7:0]
	{0x300D, 0x1F},// CPWAIT_TIME[7:0]
	{0x3050, 0x00},// ADBIT[0]
	
	{0x314C, 0xC6},// INCKSEL 1[8:0]
	{0x315A, 0x05},// INCKSEL2[1:0]
	{0x3168, 0x6B},
	
	{0x316A, 0x7D},// INCKSEL4[1:0]
	{0x319D, 0x00},// MDBIT
 ...
 *Tool ver : Ver4.0
 */
static const struct imx334_regval imx334_linear_10_3840x2160_regs[] = {
	{0x302E, 0x18},
	{0x302F, 0x0f},
 ...
 *Tool ver : Ver3.0
 */
static const struct imx334_regval imx334_hdr_10_3840x2160_regs[] = {
	{0x302E, 0x18},
	{0x302F, 0x0f},
...
static const struct imx334_regval imx334_12_3840x2160_global_regs[] = {
	{0x3001, 0x00},
	{0x3002, 0x00},
	{0x31A1, 0x00},// XVS_DRV[1:0]
	{0x3288, 0x21},// -
 ...
 *Tool ver : Ver4.0
 */
static const struct imx334_regval imx334_linear_12_3840x2160_regs[] = {
	{0x302E, 0x18},
	{0x302F, 0x0f},
	{0x3030, 0xCA},// VMAX[19:0]
	{0x3031, 0x08},//
 ...
 *Tool ver : Ver3.0
 */
static const struct imx334_regval imx334_hdr_12_74M_3840x2160_regs[] = {
	{0x302E, 0x18},
	{0x302F, 0x0f},
...
static __maybe_unused const struct imx334_regval imx334_interal_sync_master_start_regs[] = {
	{0x3010, 0x07},
	{0x31a1, 0x00},
	{IMX334_REG_NULL, 0x00},
};
static __maybe_unused const struct imx334_regval imx334_interal_sync_master_stop_regs[] = {
	{0x31a1, 0x0f},
	{IMX334_REG_NULL, 0x00},
};

static __maybe_unused const struct imx334_regval imx334_external_sync_master_start_regs[] = {
	{0x3010, 0x05},
	{0x31a1, 0x03},
	{0x31d9, 0x01},
	{IMX334_REG_NULL, 0x00},
};
static __maybe_unused const struct imx334_regval imx334_external_sync_master_stop_regs[] = {
	{0x31a1, 0x0f},
	{IMX334_REG_NULL, 0x00},
};

static __maybe_unused const struct imx334_regval imx334_slave_start_regs[] = {
	{0x3010, 0x05},
	{0x31a1, 0x0f},
	{IMX334_REG_NULL, 0x00},
};
static const struct imx334_mode imx334_supported_modes[] = {
	{
		.width = 3840,
		.height = 2160,
		.max_fps = {
...
static const s64 imx334_link_freq_menu_items[] = {
...
static int imx334_write_array(struct i2c_client *client,
			      const struct imx334_regval *regs)
{
	u32 i;
	int ret = 0;

	for (i = 0; ret == 0 && regs[i].addr != IMX334_REG_NULL; i++)
		if (unlikely(regs[i].addr == IMX334_REG_DELAY))
			  ...
			  struct v4l2_subdev_format *fmt)
{
	...
	mode = v4l2_find_nearest_size(imx334_supported_modes,
				      ARRAY_SIZE(imx334_supported_modes),
				      width, height,
	...
	if (fmt->which == V4L2_SUBDEV_FORMAT_TRY) {
		...
		*v4l2_subdev_get_try_format(sd, cfg, fmt->pad) = fmt->format;
		...
		if (imx334->cur_mipi_freq_idx != mode->mipi_freq_idx) {
			dst_pixel_rate = ((u32)imx334_link_freq_menu_items[mode->mipi_freq_idx]) /
...
{
	if (fse->index >= ARRAY_SIZE(imx334_supported_modes))
		return -EINVAL;

	if (fse->code != imx334_supported_modes[0].bus_fmt)
		return -EINVAL;

	fse->min_width = imx334_supported_modes[fse->index].width;
	fse->max_width = imx334_supported_modes[fse->index].width;
	fse->max_height = imx334_supported_modes[fse->index].height;
	fse->min_height = imx334_supported_modes[fse->index].height;
...
static long imx334_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case RKMODULE_SET_HDR_CFG:
		...
		for (i = 0; i < ARRAY_SIZE(imx334_supported_modes); i++) {
			if (w == imx334_supported_modes[i].width &&
			    h == imx334_supported_modes[i].height &&
			    imx334_supported_modes[i].hdr_mode == hdr->hdr_mode) {
				imx334->cur_mode = &imx334_supported_modes[i];
				break;
			}
		}
		if (i == ARRAY_SIZE(imx334_supported_modes)) {
	...
	case RKMODULE_GET_SYNC_MODE:
		sync_mode = (u32 *)arg;
		*sync_mode = imx334->sync_mode;
		v4l2_err(&imx334->subdev, "get sync mode %d\n",*sync_mode);
		break;
	case RKMODULE_SET_SYNC_MODE:
		sync_mode = (u32 *)arg;
		imx334->sync_mode = *sync_mode;	
		v4l2_err(&imx334->subdev, "set sync mode %d\n",*sync_mode);
		break;
	default:
		ret = -ENOIOCTLCMD;
		break;
...
static int __imx334_start_stream(struct imx334 *imx334)
{
	...
	if (imx334->sync_mode == EXTERNAL_MASTER_MODE) {
		ret |= imx334_write_array(imx334->client, imx334_external_sync_master_start_regs);
		v4l2_err(&imx334->subdev, "cur externam master mode\n");
	} else if (imx334->sync_mode == INTERNAL_MASTER_MODE) {
		ret |= imx334_write_array(imx334->client, imx334_interal_sync_master_start_regs);
		v4l2_err(&imx334->subdev, "cur intertal master\n");
	} else if (imx334->sync_mode == SLAVE_MODE) {
		ret |= imx334_write_array(imx334->client, imx334_slave_start_regs);
		v4l2_err(&imx334->subdev, "cur slave mode\n");
	}
	if (imx334->sync_mode == NO_SYNC_MODE) {
		v4l2_err(&imx334->subdev, "cur NO SYNC mode\n");
		ret = imx334_write_reg(imx334->client, IMX334_REG_CTRL_MODE,
				IMX334_REG_VALUE_08BIT, 0);
				
		usleep_range(30000, 40000);
		ret |= imx334_write_reg(imx334->client, imx334_REG_MARSTER_MODE,
					IMX334_REG_VALUE_08BIT, 0);
	} else {
		ret |= imx334_write_reg(imx334->client, imx334_REG_MARSTER_MODE,
					IMX334_REG_VALUE_08BIT, 0);
...
static int __imx334_stop_stream(struct imx334 *imx334)
{
	...
	if (imx334->sync_mode == EXTERNAL_MASTER_MODE)
		ret |= imx334_write_array(imx334->client, imx334_external_sync_master_stop_regs);
	else if (imx334->sync_mode == INTERNAL_MASTER_MODE)
		ret |= imx334_write_array(imx334->client, imx334_interal_sync_master_stop_regs);
...
static int imx334_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	const struct imx334_mode *def_mode = &imx334_supported_modes[0];
...
{
	if (fie->index >= ARRAY_SIZE(imx334_supported_modes))
		return -EINVAL;

	fie->code = imx334_supported_modes[fie->index].bus_fmt;

	fie->width = imx334_supported_modes[fie->index].width;
	fie->height = imx334_supported_modes[fie->index].height;
	fie->interval = imx334_supported_modes[fie->index].max_fps;
	fie->reserved[0] = imx334_supported_modes[fie->index].hdr_mode;
	return 0;
}
...
static int imx334_initialize_controls(struct imx334 *imx334)
{
	...
	imx334->link_freq = v4l2_ctrl_new_int_menu(handler, NULL,
						   V4L2_CID_LINK_FREQ,
						   2, 0, imx334_link_freq_menu_items);
	...
	imx334->pixel_rate = v4l2_ctrl_new_std(handler, NULL,
					       V4L2_CID_PIXEL_RATE,
					       0, IMX334_PIXEL_RATE_WITH_891M_10BIT,
					       1, dst_pixel_rate);
				  ...
				  struct i2c_client *client)
{
	...
	for (i = 0; i < 10; i++) {
		ret = imx334_read_reg(client, IMX334_REG_IMX334_CHIP_ID,
				      IMX334_REG_VALUE_16BIT, &id);
		if (id == IMX334_CHIP_ID)
			break;
	}
			...
			const struct i2c_device_id *id)
{
	...
	if (ret) {
		...
		if (strcmp(sync_mode_name, RKMODULE_EXTERNAL_MASTER_MODE) == 0)
			imx334->sync_mode = EXTERNAL_MASTER_MODE;
		else if (strcmp(sync_mode_name, RKMODULE_INTERNAL_MASTER_MODE) == 0)
			imx334->sync_mode = INTERNAL_MASTER_MODE;
		else if (strcmp(sync_mode_name, RKMODULE_SLAVE_MODE) == 0)
			imx334->sync_mode = SLAVE_MODE;
	}
	imx334->client = client;
	for (i = 0; i < ARRAY_SIZE(imx334_supported_modes); i++) {
		if (hdr_mode == imx334_supported_modes[i].hdr_mode) {
			imx334->cur_mode = &imx334_supported_modes[i];
			break;
		}
	}
	if (i == ARRAY_SIZE(imx334_supported_modes))
		imx334->cur_mode = &imx334_supported_modes[0];
