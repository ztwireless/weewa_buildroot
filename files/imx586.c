...
static int __imx586_power_on(struct imx586 *imx586)
{
	...
	if (!IS_ERR_OR_NULL(imx586->pins_default)) {
		ret = pinctrl_select_state(imx586->pinctrl,
					   imx586->pins_default);
		if (ret < 0)
			...
			const struct i2c_device_id *id)
{
	...
	imx586->pinctrl = devm_pinctrl_get(dev);
	if (!IS_ERR(imx586->pinctrl)) {
		imx586->pins_default =
			pinctrl_lookup_state(imx586->pinctrl,
					     OF_CAMERA_PINCTRL_STATE_DEFAULT);
		if (IS_ERR(imx586->pins_default))
			dev_info(dev, "could not get default pinstate\n");

		imx586->pins_sleep =
			pinctrl_lookup_state(imx586->pinctrl,
					     OF_CAMERA_PINCTRL_STATE_SLEEP);
		if (IS_ERR(imx586->pins_sleep))
